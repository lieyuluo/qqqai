...
func initQwen() {
	registerEmbeddingModel("qwen", func(ctx context.Context) (embedding.Embedder, error) {
		if config.GlobalConfig.QwenConf.BaseUrl != "" {
			defaultBaseUrl = config.GlobalConfig.QwenConf.BaseUrl
		}
		emb, err := NewEmbedder(ctx, &EmbeddingConfig{
			APIKey:  config.GlobalConfig.QwenConf.QwenKey,
			Model:   config.GlobalConfig.QwenConf.QwenEmbedding,
