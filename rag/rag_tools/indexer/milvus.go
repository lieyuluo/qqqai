	...
	registerIndexer("milvus", func(ctx context.Context) (indexer.Indexer, error) {
		...
		if err != nil {
			...
			if strings.Contains(err.Error(), "collection schema not match") {
				...
				if waitErr := waitCollectionDropped(ctx, config.GlobalConfig.MilvusConf.CollectionName, 15*time.Second); waitErr != nil {
					// 兜底：切换新集合名，避免启动失败
					newName := fmt.Sprintf("%s_%d", config.GlobalConfig.MilvusConf.CollectionName, time.Now().Unix())
					log.Printf("旧集合仍存在，改用新集合: %s", newName)
					config.GlobalConfig.MilvusConf.CollectionName = newName
				}
				indexer, err = milvus.NewIndexer(ctx, buildMilvusIndexerConfig(dim))
				if err != nil {
					return nil, err
